"""GraphDB uploader for CIM data.

Uploads RDF data to an Ontotext GraphDB repository via the RDF4J
statements endpoint. The first instantiation against a fresh container
auto-creates the target repository (see `GraphDBConnection.configure`).
"""

from __future__ import annotations

import logging

import requests

from cimloader._base_iri import DEFAULT_BASE_IRI, prepare_rdf_bytes
from cimloader._formats import content_type_from_filename, content_type_from_url
from cimloader.databases import GraphDBConnection

_log = logging.getLogger(__name__)


class GraphDBUploader(GraphDBConnection):
    def __init__(self, base_iri: str = DEFAULT_BASE_IRI) -> None:
        super().__init__()
        self.base_iri = base_iri
        self.configure()

    def upload_from_file(self, filepath: str, filename: str) -> None:
        """Upload an RDF file to GraphDB.

        Format is auto-detected from the file extension. RDF/XML files
        get `xml:base=<self.base_iri>` injected when they don't already
        declare one, so fragment IRIs (`#_UUID`) resolve deterministically.
        """
        content_type = content_type_from_filename(filename)
        with open(f"{filepath}/{filename}", "rb") as f:
            data = prepare_rdf_bytes(f.read(), content_type, self.base_iri)
        _log.info("Uploading %s to GraphDB at %s", filename, self.statements_endpoint)
        resp = requests.post(
            self.statements_endpoint,
            data=data,
            headers={"Content-Type": content_type},
        )
        resp.raise_for_status()
        _log.info("Successfully uploaded %s to GraphDB", filename)

    def upload_from_url(self, url: str) -> None:
        """Fetch an RDF file from a URL and upload it to GraphDB.

        Format is auto-detected from the URL path extension.
        """
        content_type = content_type_from_url(url)
        _log.info("Fetching %s for upload to GraphDB", url)
        resp = requests.get(url)
        resp.raise_for_status()
        data = prepare_rdf_bytes(resp.content, content_type, self.base_iri)
        post = requests.post(
            self.statements_endpoint,
            data=data,
            headers={"Content-Type": content_type},
        )
        post.raise_for_status()
        _log.info("Successfully uploaded %s to GraphDB", url)

    def upload_from_graphmodel(self, graph_dict: dict, feeder_mrid: str | None = None) -> None:
        """Upload a CIMantic Graphs GraphModel to GraphDB."""
        from cimgraph.models import FeederModel

        if self.cim is None:
            raise RuntimeError(
                "CIM profile not configured. Set CIMG_CIM_PROFILE environment variable."
            )

        if feeder_mrid:
            container = self.cim.Feeder(mRID=feeder_mrid)
        else:
            import uuid
            container = self.cim.Feeder(mRID=str(uuid.uuid4()))

        _log.info("Uploading graph with %d object types to GraphDB", len(graph_dict))
        FeederModel(container=container, connection=self, graph=graph_dict)
